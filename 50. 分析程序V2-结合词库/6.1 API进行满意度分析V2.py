...
MODEL = "deepseek-chat"  # 通用聊天模型（性价比高）
MAX_WORKERS = 10  # 并发线程数
RETRY_TIMES = 3  # 重试次数
RETRY_DELAY = 1  # 重试间隔（秒）
REQUEST_DELAY = 0.1  # 请求休眠（避免限流）
...
5. 添加字段 "experience_reason"，简要说明为什么将该方面归类为 "pre-purchase" 或 "long-term"（例如：因为该方面在购车前即可通过静态体验或试驾感受到，或者需要长期使用才能评估）。
...
# ========== 3. 输出目录 ==========
OUTPUT_DIR = "最满意API分析_V2"  # 输出文件夹
os.makedirs(OUTPUT_DIR, exist_ok=True)

INPUT_EXCEL = "../10.汽车口碑数据/merged_autohome_reviews_V2.xlsx"  # 输入文件（可根据需要调整）
OUTPUT_EXCEL = os.path.join(OUTPUT_DIR, "购买原因分析结果.xlsx")
DETAIL_EXCEL = os.path.join(OUTPUT_DIR, "分类结果明细.xlsx")  # 全量明细文件
...
# ========== 5. 定义 API 调用函数（支持重试，增加评分验证，并提取 experience_reason）==========
    ...
    for attempt in range(retries):
        try:
            ...
            if isinstance(result, list):
                validated = []
                for item in result:
                    # 验证字段有效性，包括评分和体验类型，以及新增的体验理由（可选）
...
# ========== 6. 多线程批量处理 ==========
results = []  # 所有分类结果（每条记录对应一个类别-情感）
failed_indices = []  # 完全失败的评论索引

print("开始批量分析（多线程）...")
comment_items = [(i, row["车型名称"], row["最满意"]) for i, row in df.iterrows()]  # 注意列名
        ...
        if categories:
            for cat in categories:
                results.append({
                    ...
                    "体验类型理由": cat.get("experience_reason", ""),  # 【新增】
                    "评论内容": comment
                })
        else:
...
if results_df_this.empty:
    ...
    # 新增：计算非常不满意（评分=-2）和非常满意（评分=2）的次数
    ...
    # 按车型+类别统计非常不满意（评分=-2）和非常满意（评分=2）的次数
    ...
    exp_type_dist = pd.DataFrame({
        "提及次数": exp_type_counts,
        "占比(%)": exp_type_pct
    })
    print("\n整体体验类型分布:")
    print(exp_type_dist)
    ...
    # 10.2 各类别在不同体验类型中的提及次数（堆叠条形图）
    # 选取 top10 类别（按总提及）
    top_cats = category_stats.head(10).index.tolist()
    exp_cat_top = exp_cat_counts[top_cats].T  # 转置以便绘图
    ...
    plt.title("各车型购买原因类别提及次数热力图（绝对频数）")
    plt.tight_layout()
